...
class Logger:
    ...
    def set_handlers(self, console_bool, file_bool):
        ...
        if file_bool:
            # Add file handler
            fh = logging.FileHandler(f'{self.log_file_name}{datetime.datetime.now():%Y-%m-%d_%H.%M}.log')
            fh.setLevel(logging.DEBUG)
            ff = OneLineFormatter('[%(levelname)s] [%(asctime)s] [%(name)s] - %(message)s')
            fh.setFormatter(ff)
            self.log.addHandler(fh)

    def reset_file_handler(self):
        fh = [
            i for i in self.log.handlers if isinstance(i, logging.FileHandler)][0]
